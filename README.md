...
But what if you don't care about the max and are more interested in the **Minimum LOS Orbit**? We've got you covered there, too! *(For constellation with 3 or more satellites only...)*. You can even account for **Occlusion** due to a planet's atmosphere (if it's got one). Toggle Occlusion on will allow you to set the **Atm** and **Vac** factors to apply, and will automatically apply the appropriate one for the body you're orbitting to set the Min LOS Orbit altitude accordingly.
