...
namespace ResonantOrbitCalculator.UI;
...
public class FoldOut
{
    ...
    public void OnGui()
    {
        ...
        for (int i = 0; i < chapters.Count; i++)
        {
            ...
            var style = chapter.opened ? ROCStyles.foldout_open : ROCStyles.foldout_close;
...
public class TopButtons
{
    ...
    {
        position.x -= space;
        return GUI.Button(position, txt, ROCStyles.icon_button);
    }
    static public bool IconButton(Texture2D icon)
    {
        position.x -= space;
        return GUI.Button(position, icon, ROCStyles.icon_button);
    }

    static public bool Toggle(bool value, string txt)
    {
        position.x -= space;
        return GUI.Toggle(position, value, txt, ROCStyles.icon_button);
    }

    static public bool Toggle(bool value, Texture2D icon)
    {
        position.x -= space;
        return GUI.Toggle(position, value, icon, ROCStyles.icon_button);
    }
}
...
public class UI_Tools
{
    ...
    {
        if (tooltip != null)
            return GUILayout.Toggle(is_on, new GUIContent(txt, tooltip), ROCStyles.toggle);
        else
            return GUILayout.Toggle(is_on, txt, ROCStyles.toggle);
    }
    ...
    public static bool ToggleButton(bool is_on, string txt_run, string txt_stop)
    {
        ...
        is_on = GUILayout.Toggle(is_on, txt, ROCStyles.big_button, GUILayout.Height(height_bt), GUILayout.MinWidth(min_width_bt));
        // GUILayout.FlexibleSpace();
        // GUILayout.EndHorizontal();
        return is_on;
    }
    ...
    {
        return GUILayout.Button(txt, ROCStyles.big_button);
    }

    public static bool SmallButton(string txt)
    {
        return GUILayout.Button(txt, ROCStyles.small_button);
    }
     
    public static bool BigButton(string txt)
    {
        return GUILayout.Button(txt, ROCStyles.bigicon_button);
    }

    public static bool ListButton(string txt)
    {
        return GUILayout.Button(txt, ROCStyles.button, GUILayout.ExpandWidth(true));
    }

    public static bool miniToggle(bool value, string txt, string tooltip)
    {
        return GUILayout.Toggle(value, new GUIContent(txt, tooltip), ROCStyles.small_button, GUILayout.Height(20));
    }

    public static bool miniButton(string txt, string tooltip = "")
    {
        return GUILayout.Button(new GUIContent(txt, tooltip), ROCStyles.small_button, GUILayout.Height(20));
    }

    public static bool ToolTipButton(string tooltip)
    {
        return GUILayout.Button(new GUIContent("?", tooltip), ROCStyles.small_button, GUILayout.Width(16), GUILayout.Height(20));
    }

    static public bool BigIconButton(Texture2D icon)
    {
        return GUILayout.Button(icon, ROCStyles.bigicon_button);
    }


    public static void Title(string txt)
    {
        GUILayout.Label($"<b>{txt}</b>", ROCStyles.title);
    }

    public static void Label(string txt)
    {
        GUILayout.Label(txt, ROCStyles.label);
    }

    public static void OK(string txt)
    {
        GUILayout.Label(txt, ROCStyles.phase_ok);
    }

    public static void Warning(string txt)
    {
        GUILayout.Label(txt, ROCStyles.phase_warning);
    }

    public static void Error(string txt)
    {
        GUILayout.Label(txt, ROCStyles.phase_error);
    }
    ...
    {
        GUILayout.Label(txt, ROCStyles.console_text);
    }

    public static void Mid(string txt)
    {
        GUILayout.Label(txt, ROCStyles.mid_text);
    }
    ...
    public static int IntSlider(string txt, int value, int min, int max, string postfix = "", string tooltip = "")
    {
        ...
        GUILayout.Label(content, ROCStyles.slider_text);
        GUILayout.BeginHorizontal();
        value = (int) GUILayout.HorizontalSlider((int) value, min, max, ROCStyles.slider_line, ROCStyles.slider_node);
    ...
    public static float HeadingSlider(string txt, float value, string tooltip = "")
    {
        ...
        GUILayout.Label(content, ROCStyles.slider_text);
        GUILayout.BeginHorizontal();
        value = GUILayout.HorizontalSlider( value, -180, 180, ROCStyles.slider_line, ROCStyles.slider_node);
    ...
    {
        GUILayout.Box("", ROCStyles.separator);
    }
    ...
    public static void ProgressBar(float value, float min, float max)
    {
        ...
        GUILayout.Box("", ROCStyles.progress_bar_empty, GUILayout.ExpandWidth(true));
        var lastrect = GUILayoutUtility.GetLastRect();

        lastrect.width = Mathf.Clamp(lastrect.width * ratio, 4, 10000000);
        GUI.Box(lastrect, "", ROCStyles.progress_bar_full);
    }
    ...
    public static float FloatSlider(float value, float min, float max, string tooltip = "")
    {
        // simple float slider
        GUILayout.BeginHorizontal();
        value = GUILayout.HorizontalSlider( value, min, max, ROCStyles.slider_line, ROCStyles.slider_node);
